...
def dessine_cases(surface_plateau: pygame.Surface, déplacements: list, terrainEnBas, color_=JAUNE_SYMPA):
    ...
    for typeDép, cases in déplacements:
        # print(cases)
        for p, d, g in cases:
            origine = (centre + c * 6 * np.exp(
                (np.pi / 2 + 2 * ((p - terrainEnBas) % 3) * np.pi / 3) * 1j))  # Point (0,0) pour le terrain p
            z = origine + c * ((d - g) * (np.sqrt(3) / 2) - (1 + d + g) * 1j / 2) * np.exp(
                ((p - terrainEnBas) % 3) * 2j * np.pi / 3)
...
def affichage(nom_joueur, joueur_0, joueur_1, joueur_2, sur_déplacement_pièce=None, sur_attente_autres_joueurs=None):
    ...
    pygame.display.set_caption("Échecs à 3")
        ...
        for d in range(6):
            for g in range(6):
                ...
                if isinstance(case, Piece):  # A faire si une pièce est à l'emplacement de la case
                    ...
                    z = décaleImage + centre + np.exp(2j * ((p - terrainEnBas) % 3) * np.pi / 3) * (
                                centreCase + d * u + g * v - centre)
...
def detecte_terrain_curseur(centre, event, largeurPlateau, hauteurPlateau, xPlateau, yPlateau, terrainEnBas):
    ...
    if yCurseur < haut1(xCurseur) or yCurseur < haut2(xCurseur) or yCurseur > bas1(xCurseur) or yCurseur > bas2(
            xCurseur) or xCurseur < xPlateau or xCurseur > xPlateau + largeurPlateau:
